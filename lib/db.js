...
export const db = {
    getAllItems: async (collectionSlug) => {
        try {
            ...
            if (conn) {
                // Query the new Product model
                let query = { isActive: true };

                // If specific collection selected (and not 'All')
                if (collectionSlug && collectionSlug !== 'All' && collectionSlug !== 'all') {
                    ...
                    if (collectionDoc) {
                        // Find this collection AND all its children
                        const childCollections = await Collection.find({ parentCollection: collectionDoc._id });
                        const allCollectionIds = [collectionDoc._id, ...childCollections.map(c => c._id)];
